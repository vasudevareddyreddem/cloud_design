
    <!-- start js include path -->
    <script src="assets/jquery.min.js" ></script>
    <script src="assets/tether/js/tether.min.js" ></script>
    <script src="assets/jquery.blockui.min.js" ></script>
	<script src="assets/jquery.slimscroll.js"></script>
    <!-- bootstrap -->
    <script src="assets/bootstrap/js/bootstrap.min.js" ></script>
    <script src="assets/bootstrap-switch/js/bootstrap-switch.min.js" ></script>
    <!-- counterup -->
	<!-- data tables -->
    <script src="assets/datatables/jquery.dataTables.min.js" ></script>
 	<script src="assets/datatables/plugins/bootstrap/dataTables.bootstrap4.min.js" ></script>
    <script src="assets/table_data.js" ></script>
    <!-- Common js-->
    <script src="assets/counterup/jquery.waypoints.min.js" ></script>
    <script src="assets/counterup/jquery.counterup.min.js" ></script>
	<script  src="assets/wizard/js/jquery.smartWizard.min.js"></script>
    <script  src="assets/wizard/js/wizard-data.js"></script>
    <!-- Common js-->
	<script src="assets/bootstrap-datetimepicker/js/bootstrap-datetimepicker.js"  charset="UTF-8"></script>
    <script src="assets/bootstrap-datetimepicker/js/bootstrap-datetimepicker-init.js"  charset="UTF-8"></script>
	<script src="assets/app.js" ></script>
    <script src="assets/layout.js" ></script>
    <script src="assets/theme-color.js" ></script>
    <!-- material -->
    <script src="assets/material/material.min.js"></script>
	<script src="assets/chart-js/Chart.bundle.js" ></script>
    <script src="assets/chart-js/utils.js" ></script>
    <script src="assets/chart-js/home-data2.js" ></script>
    <script src="assets/sparkline/jquery.sparkline.js" ></script>
	<script src="assets/sparkline/sparkline-data.js" ></script>
	<script src="assets/select2/js/select2.js" ></script>
    <script src="assets/select2/js/select2-init.js" ></script>
  </body>

</html>